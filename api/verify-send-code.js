...
function normalizePhone(raw) {
  // Accepte 06xxxxxxxx / 6xxxxxxxx / +336xxxxxxxx, renvoie toujours +33...
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!twilioRes.ok) {
      console.error("verify-send-code Twilio error:", data);
      // Message générique côté client, détail en log serveur uniquement
      return res.status(400).json({ success: false, error: "Impossible d'envoyer le code. Vérifiez le numéro saisi." });
    }

    return res.status(200).json({ success: true, status: data.status, phone });
  } catch (err) {
